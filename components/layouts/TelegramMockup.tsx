...
const TelegramMockup: React.FC<{ postData: PostData }> = ({ postData }) => {
  ...
  return (
    <div className={`w-[450px] ${isDark ? 'bg-[#17212b]' : 'bg-[#73a0c5]'} p-4 shadow-2xl rounded-xl min-h-[400px] flex flex-col justify-end`}>
       <div className={`p-0 overflow-hidden ${isDark ? 'bg-[#182533] text-white' : 'bg-white text-black'} rounded-xl shadow-sm max-w-[90%] self-start relative group`}>
         ...
         <div className="p-3 pb-6">
           <div className="font-bold text-[#3390ec] mb-1">{postData.identity.name}</div>
           <div className="text-sm whitespace-pre-wrap">{postData.content.text}</div>
         </div>
